...
不同工具创建加密包时, 密码会按不同编码存为字节:
  - Windows 传统工具 (7z.exe/WinRAR in 中文系统): 按 ACP(GBK)
  - Android / 现代工具 (MT管理器, 部分手机App)   : 按 UTF-8
繁体系统工具可能按 Big5, 日文工具按 Shift-JIS.
7z.dll 收到宽字符密码后, 内部按当前代码页(ACP)转字节, 因此对非 ACP 编码密码的包, 直接传宽字符会失败.
解法: 通过 -enc 参数构造"按指定编码转字节, 再按 ACP 解码"的宽字符变体,7z.dll 把它按 ACP 编码回去恰好还原目标编码的原始字节, 即可解开.
