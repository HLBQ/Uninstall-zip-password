...
支持通过 -enc 指定密码编码变体（如 utf8），解决 Android / 手机工具（MT管理器等）按 UTF-8 字节创建的加密包打不开的问题。；不写 -enc 时仅按系统代码页原样尝试，不会回退到其他编码，因此密码总数不因编码变体而改变。
...
| `-i <类型>` | zip | 压缩格式：`zip`/`7z`/`bz2`/`xz`/`wim`/`tar`/`gz` **注意：标准做法应一次指定一种编码**|
| `-d` | 否 | 打包后删除源文件 |
...
| `-ps <密码>` | 解压密码（可选） |
| `-k <类型>` | 强制指定压缩格式 |
| `-enc <编码>` | 密码编码变体，**支持编码**：`utf8`/`gbk`(默认) /`gb2312`/`big5`/`sjis` **注意：标准做法应一次指定一种编码**|
