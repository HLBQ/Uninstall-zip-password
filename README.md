...
```
7z_to_dll.exe -e <压缩包> [-o <目录>] [-ps <密码>] [-k <类型>] [-enc <编码>] [-iv] [-sniff] [-d]
```
...
| `-iv` | 忽略验证：跳过整体 test() 直接解压 |
| `-sniff` | 魔数嗅探优先于扩展名，应对伪装扩展名的特殊包|
...
7z_to_dll.exe -e 损坏的压缩包.zip -o out -ps 1234 -iv
7z_to_dll.exe -e 7z压缩包.zip -o out -ps 1234 -sniff
```

### 破解模式

```
7z_to_dll.exe <压缩包> -c <字符集> -ts N -te N [-p <密码本>] [-s N] [-e N] [-k <类型>] [-b|-nb] [-enc <编码>] [-lv] [-sniff]
7z_to_dll.exe <压缩包> -p <密码本> [-s N] [-e N] [-b|-nb] [-enc <编码>] [-lv] [-sniff]
```
...
| `-nb` | 自动 | 禁用内存缓存 |
| `-enc <编码>` | — | 密码编码变体，**支持编码**：`utf8` /`gbk`(默认) /`gb2312` /`big5` /`sjis` |
| `-lv` | 否 | 轻量验证：仅读头不解压，应对含损坏条目的特殊包，更快但可能假阳性 |
| `-sniff` | 否 | 魔数嗅探优先于扩展名，应对伪装扩展名的特殊包 |
...
:: 轻量验证（含损坏条目的包破解更快，但可能误判）
7z_to_dll.exe 特殊压缩包4.zip -p 密码.txt -lv

:: 魔数嗅探（伪装扩展名的包，如 .7z 实为 ZIP）
7z_to_dll.exe 特殊压缩包5.7z -p 密码.txt -sniff
```
