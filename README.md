...
支持字符集组合、密码本逐行尝试以及两者的混合模式。内置 **O(1) 断点恢复机制**，可精确跳转到任意密码位置，无需迭代。
...
```
7z_to_dll.exe -e <压缩包> [-o <目录>] [-ps <密码>] [-k <类型>] [-d]
```
...
```
7z_to_dll.exe <压缩包> -c <字符集> -ts N -te N [-p <密码本>] [-s N] [-e N] [-k <类型>] [-b|-nb] [-tr N] [-bt N]
7z_to_dll.exe <压缩包> -p <密码本> [-s N] [-e N] [-b|-nb] [-tr N] [-bt N]
```
...
| `-b` | 自动 | 强制缓存压缩包到内存 |
| `-nb` | 自动 | 禁用内存缓存 |
| `-tr N` | 自动 | 线程数；`0`=自动（CPU 核心数），`N`=指定 N 个线程 |
| `-bt N` | 2048 | 多线程下每批次任务数 |
...
:: 多线程暴力破解（8线程，批次512）
7z_to_dll.exe 测试.7z -c abc.txt -ts 5 -te 6 -tr 8 -bt 512

:: 纯密码本多线程（密码本优先，暴力阶段使用多线程）
7z_to_dll.exe 测试.7z -c abc.txt -ts 1 -te 4 -p pb.txt -tr 8
```
...
| 参数 | 作用 | 建议值 |
|------|------|--------|
| `-tr N` | Worker 线程数 | CPU 核心数（默认自动） |
| `-bt N` | 每批任务数 | 2048（默认），大任务量可调高减少竞争 |
